...
def main_loop(mapa, posicion_inicial, posicion_final):
    ...
    while (px, py) != posicion_final:
        mapa[py][px] = 'P'  # Marcamos la posición del jugador en el mapa
        mostrar_mapa(mapa)

        tecla = input("Utiliza las teclas de flechas para moverte. Presiona Enter para continuar...")
        clear_terminal()
        
        if tecla == 'w' and py > 0 and mapa[py - 1][px] != '#':
            mapa[py][px] = '.'  # Restauramos el espacio vacío
            py -= 1
        elif tecla == 's' and py < len(mapa) - 1 and mapa[py + 1][px] != '#':
            mapa[py][px] = '.'  # Restauramos el espacio vacío
            py += 1
        elif tecla == 'a' and px > 0 and mapa[py][px - 1] != '#':
            mapa[py][px] = '.'  # Restauramos el espacio vacío
            px -= 1
        elif tecla == 'd' and px < len(mapa[0]) - 1 and mapa[py][px + 1] != '#':
            mapa[py][px] = '.'  # Restauramos el espacio vacío
            px += 1
